...
class RequestAdapter:
    ...
    def convert(self, raw_xml: str) -> dict:
        ...
        result_dict = {key: value for key, value in self.default_values.items()}

        for key, value in bikeCad_file_entries.items():
            model_key = self.bikeCad_to_model_map.get(key, key)
            if model_key in result_dict.keys():
                result_dict[model_key] = value

        result_dict["Material=Steel"] = 1
        result_dict["Material=Titanium"] = 0
        result_dict["Material=Aluminum"] = 0
