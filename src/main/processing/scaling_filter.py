...
class ScalingFilter:
    ...
    def scale(self, data: dict) -> dict:
        return self.__operate_on_dict(data, self.scale_dataframe)

    def unscale(self, data: dict) -> dict:
        return self.__operate_on_dict(data, self.unscale_dataframe)

    def scale_dataframe(self, unscaled_data: pd.DataFrame) -> pd.DataFrame:
        return self.__operate_on_dataframe(unscaled_data, self.scaler.transform)
    ...
    def __operate_on_dataframe(self, dataframe: pd.DataFrame, operation: callable) -> pd.DataFrame:
        ...
        return self.__rebuild_dataframe(new_values, dataframe, missing_columns)

    def __rebuild_dataframe(self, values, original_dataframe, missing_columns):
        return pd.DataFrame(values,
                            columns=original_dataframe.columns,
                            index=original_dataframe.index).drop(columns=missing_columns)

    def __reorder_dataframe(self, dataframe):
